...
async fn create_new_district(Json(district): Json<District>) -> String{
    ...
    let db = Surreal::new::<Wss>("generalpione.preciqprojects.com").await.expect("Error: Unable to connect to surreal db instance");
    db.signin(Root {
        username: "root",
        password: "test12345",
    }).await.expect("Error: Unable to login to Surreal DB instance with credentials.");

    db.use_ns("bd").use_db("bd").await.expect("Error: Unable to to specified namespace/database");

    let new_entry: Vec<Record> = db.create("table_of_districts_rana").content(district).await
